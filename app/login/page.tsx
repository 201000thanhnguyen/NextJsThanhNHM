...
function LoginInner() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const role = parseRole(data.user?.role) ?? "USER";
      const dest = postLoginDestination(role, nextParam);
      await refresh();
      router.replace(dest);
      router.refresh();
    } catch (err) {
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gray-600">
          Sign in with your account. Administrators are sent to the management console;
          other users return to the public site.
        </p>
